...
function read_input()::Array{String}
    ...
    current_passport::Array{String} = []
    open("./input", "r") do f
        while ! eof(f)
            s::String = readline(f)
            if s == ""
                pass_info = join(current_passport, " ")
                push!(passports,pass_info)
                current_passport = []
            else
                push!(current_passport,s)
            end
        end
    end
...
function count_valid_passports(passports::Array{String})::Int32
